...
function renderEmail(inv, attachmentName) {
  ...
  const lines = items.map((it) => {
    ...
    const w = (Number(it.warranty_days) > 0 && it.warranty_type !== "none")
      ? `<div style="font-size:11px;color:#5f6b7d;margin-top:4px;border-left:3px solid ${bar};padding-left:7px;line-height:1.45">${esc(wLabel(it.warranty_type))} <b style="color:#141b25">${Number(it.warranty_days)}</b> days${lineShowsExpiry(it) ? ` &middot; valid to ${niceDate(it.warranty_until)}` : ""}</div>`
      : "";
    return `<tr>
