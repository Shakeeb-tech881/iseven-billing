/* =========================================================
   ...
   Environment variables:
     ...
     MAIL_FROM   I7SEVEN MOBILES <info@iseven.lk>
...
const FROM = () => process.env.MAIL_FROM || "I7SEVEN MOBILES <info@iseven.lk>";
...
/* Whether "365 days" is printed. Older invoices have no value and keep
   showing it, as they always did. */
const lineShowsDays = (it) => !(it.show_days === false || it.show_days === 0);

/* A pack item's cover, worded to read naturally whichever of the two
   parts is switched off. sep is the separator for HTML or plain text. */
function packTail(inv, it, days, until, sep) {
  const d = lineShowsDays(it), e = lineShowsExpiry(inv, it) && until;
  if (d && e) return `${days} days warranty${sep}valid to ${niceDate(until)}`;
...
function renderEmail(inv, attachmentName) {
  ...
  const lines = items.map((it) => {
    ...
    const pack = packLines.length
      ? `<div style="margin-top:4px;font-size:11px;color:#5f6b7d;line-height:1.5">${
          packLines.map((l) => {
            const pi = parsePackLine(l);
            const sn = pi.serial
              ? ` <span style="font-family:Consolas,monospace;font-size:10px">${esc(serialLabel(pi.serial))}</span>` : "";
            if (!pi.days) return `<div>&bull;&nbsp; ${esc(pi.name)}${sn}</div>`;
            const pUntil = addDaysIso(inv.issue_date, pi.days);
            const tail = packTail(inv, it, pi.days, pUntil, " &middot; ");
            return `<div>&bull;&nbsp; ${esc(pi.name)}${sn} <b style="color:#141b25">${tail}</b></div>`;
          }).join("")}</div>`
      : "";
    ...
    const w = (Number(it.warranty_days) > 0 && it.warranty_type !== "none")
      ? `<div style="font-size:11px;color:#5f6b7d;margin-top:4px;border-left:3px solid ${bar};padding-left:7px;line-height:1.45">${esc(wLabel(it.warranty_type))}${lineShowsDays(it) ? ` <b style="color:#141b25">${Number(it.warranty_days)}</b> days` : ""}${lineShowsExpiry(inv, it) ? ` &middot; valid to ${niceDate(it.warranty_until)}` : ""}</div>`
      : "";
    return `<tr>
      <td style="padding:9px 0;border-bottom:1px solid #e7eaf1;font-size:13px;color:#141b25">
        ${esc(it.description)}${imei}${pack}${w}
      </td>
...
<table width="640" cellpadding="0" cellspacing="0" style="max-width:640px;background:#ffffff">
  ...
  <tr><td style="background:#10161f;padding:20px 32px">
    <table width="100%" cellpadding="0" cellspacing="0"><tr>
      <td valign="top">
        <img src="cid:i7logo" alt="I7SEVEN MOBILES" width="170" style="display:block;border:0">
      </td>
...
function renderText(inv) {
  const cur = inv.currency || "LKR";
  const L = [];
  L.push(`${inv.biz_name || "I7SEVEN MOBILES"}`);
  ...
  for (const it of (inv.items || [])) {
    ...
    for (const raw of String(it.pack_items || "").split("\n")) {
      if (!raw.trim()) continue;
      const pi = parsePackLine(raw);
      const sn2 = pi.serial ? `  ${serialLabel(pi.serial)}` : "";
      if (!pi.days) { L.push(`   - ${pi.name}${sn2}`); continue; }
      const pUntil = addDaysIso(inv.issue_date, pi.days);
      L.push(`   - ${pi.name}${sn2} (${packTail(inv, it, pi.days, pUntil, ", ")})`);
    }
    if (Number(it.warranty_days) > 0 && it.warranty_type !== "none") {
      const exp = lineShowsExpiry(inv, it)
        ? `, valid to ${niceDate(it.warranty_until)}` : "";
      const dys = lineShowsDays(it) ? ` ${it.warranty_days} days` : "";
      L.push(`   ${wLabel(it.warranty_type)}${dys}${exp}`);
    }
  }
  L.push("");
...
function invoiceAttachment(inv) {
  const html = renderEmail(inv, `${inv.number}.html`)
    .replace('<img src="cid:i7logo" alt="I7SEVEN MOBILES" width="170" style="display:block;border:0">',
      '<div style="font-family:Arial,Helvetica,sans-serif;font-size:26px;font-weight:bold;font-style:italic;color:#c6fa02;letter-spacing:-1px">I7SEVEN MOBILES</div>')
    .replace("</body>",
...
async function sendInvoiceEmail(inv) {
  ...
  const info = await getTransport().sendMail({
    ...
    subject: `${inv.tax_mode === "none" ? "Invoice" : "Tax invoice"} ${inv.number} from ${inv.biz_name || "I7SEVEN MOBILES"}`,
    text: renderText(inv),
