...
CREATE TABLE IF NOT EXISTS invoices (
  ...
  terms         TEXT,
  cashier       TEXT,
  show_warranty_expiry INTEGER NOT NULL DEFAULT 1,
...
for (const [name, ddl] of [
  ["show_warranty_expiry", "ALTER TABLE invoices ADD COLUMN show_warranty_expiry INTEGER NOT NULL DEFAULT 1"],
...
function createInvoice(body) {
  ...
  const ins = db.prepare(`
    INSERT INTO invoices (
      ...
      warranty_text, terms, cashier, show_warranty_expiry, created_at
    ) VALUES (?,?,?,?, ?,?,?,?,?, ?,?,?,?, ?,?, ?,?,?,?,?,?,?,?, ?,?,?,?,?)`);

  const insItem = db.prepare(`
    INSERT INTO items (invoice_id,pos,description,imei,warranty_type,warranty_days,
                       warranty_until,qty,unit_price_c,amount_c,show_expiry)
    VALUES (?,?,?,?,?,?,?,?,?,?,?)`);
  ...
  try {
    ...
    const info = ins.run(
      ...
      n.terms ?? st.terms,
      n.cashier, n.show_warranty_expiry ? 1 : 0, new Date().toISOString()
    );
    ...
    for (const it of n.items) {
      insItem.run(id, it.pos, it.description, it.imei, it.warranty_type,
                  ...
                  it.unit_price_c, it.amount_c, it.show_expiry ? 1 : 0);
    }
    db.exec("COMMIT");
