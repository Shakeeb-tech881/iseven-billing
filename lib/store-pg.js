/* =========================================================
   I7SEVEN MOBILES — PostgreSQL / Supabase backend
  ...
  try {
    ...
    const ph = n.items.map((it, i) => {
      ...
      vals.push(invId, it.pos, it.description, it.imei, it.warranty_type,
                ...
                it.show_expiry, it.pack_items, it.show_days);
    ...
    await client.query(`INSERT INTO items (invoice_id,pos,description,imei,warranty_type,
                        warranty_days,warranty_until,qty,unit_price_c,amount_c,show_expiry,pack_items,show_days)
                        VALUES ${ph}`, vals);
  ...
  try {
    ...
    await client.query(`
      UPDATE invoices SET
        ...
        warranty_text=$21, terms=$22, cashier=$23
      WHERE id=$24`, [
...
async function searchInvoices({ q: term, field, from, to, limit, createdSince }) {
  ...
  if ((term || "").trim()) {
    if (field === "nic")         where.push(`i.cust_nic ILIKE ${add(like)}`);
      ...
      /* A serial written inside a pack must be findable too, or a
         warranty claim on a pack item cannot be looked up. */
      const p1 = add(like), p2 = add(like);
      where.push(`EXISTS (SELECT 1 FROM items x WHERE x.invoice_id = i.id
                    AND (x.imei ILIKE ${p1} OR x.pack_items ILIKE ${p2}))`);
    }
      ...
      where.push(`(i.cust_nic ILIKE ${p} OR i.number ILIKE ${p} OR i.cust_name ILIKE ${p}
        ...
        OR EXISTS (SELECT 1 FROM items x WHERE x.invoice_id = i.id
             AND (x.imei ILIKE ${p} OR x.pack_items ILIKE ${p})))`);
    }
  }
  if (from) where.push(`i.issue_date >= ${add(from)}`);
  if (to)   where.push(`i.issue_date <= ${add(to)}`);
  /* Cashiers see only invoices created today. */
  if (createdSince) where.push(`i.created_at >= ${add(createdSince)}`);
...
async function recordEmail(id, status, error, messageId) {
  await q(`UPDATE invoices
              SET email_status = $2, email_error = $3,
                  ...
                  email_delivered_at = CASE WHEN $2 = 'sent' THEN NULL ELSE email_delivered_at END
            WHERE id = $1`, [id, status, error || null, messageId || null]);
}

/* Brevo tells us afterwards whether the message actually arrived.
   Matched on its message id; if that does not line up (an SMTP relay may
   rewrite it) we fall back to the most recent invoice sent to that
   address, which is what a shop wants anyway. */
async function recordDelivery({ messageId, email, status, reason, at }) {
  const set = `email_status = $1, email_error = $2,
               email_delivered_at = CASE WHEN $1 = 'delivered' THEN $3 ELSE email_delivered_at END`;
  if (messageId) {
    const r = await q(`UPDATE invoices SET ${set}
                        WHERE email_message_id = $4 RETURNING id`,
                      [status, reason || null, at, messageId]);
    if (r.rows.length) return r.rows.map((x) => num(x.id));
  }
  if (!email) return [];
  const r2 = await q(`UPDATE invoices SET ${set}
                       WHERE id = (SELECT id FROM invoices
                                    WHERE lower(cust_email) = lower($4)
                                      AND email_sent_at IS NOT NULL
                                    ORDER BY email_sent_at DESC LIMIT 1)
                     RETURNING id`, [status, reason || null, at, email]);
  return r2.rows.map((x) => num(x.id));
}
...
module.exports = { getPool, getSettings, putSettings, createInvoice, updateInvoice,
                   ...
                   recordDelivery, healthCheck, describe };
