/* =========================================================
   I7SEVEN MOBILES — SQLite backend
...
CREATE TABLE IF NOT EXISTS items (
  ...
  show_days      INTEGER NOT NULL DEFAULT 1,
  pack_items     TEXT,
...
if (!icols.has("show_expiry")) {
  db.exec("ALTER TABLE items ADD COLUMN show_expiry INTEGER NOT NULL DEFAULT 1");
}
if (!icols.has("pack_items")) {
  db.exec("ALTER TABLE items ADD COLUMN pack_items TEXT");
}
const vcols = new Set(db.prepare("PRAGMA table_info(invoices)").all().map((r) => r.name));
if (!vcols.has("email_message_id")) {
  db.exec("ALTER TABLE invoices ADD COLUMN email_message_id TEXT");
}
if (!vcols.has("email_delivered_at")) {
  db.exec("ALTER TABLE invoices ADD COLUMN email_delivered_at TEXT");
}
if (!icols.has("show_days")) {
  db.exec("ALTER TABLE items ADD COLUMN show_days INTEGER NOT NULL DEFAULT 1");
}
...
function createInvoice(body) {
  ...
  const insItem = db.prepare(`
    INSERT INTO items (invoice_id,pos,description,imei,warranty_type,warranty_days,
                       warranty_until,qty,unit_price_c,amount_c,show_expiry,pack_items,show_days)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`);
  ...
  try {
    ...
    for (const it of n.items) {
      insItem.run(id, it.pos, it.description, it.imei, it.warranty_type,
                  ...
                  it.show_expiry ? 1 : 0, it.pack_items, it.show_days ? 1 : 0);
    }
    db.exec("COMMIT");
...
function searchInvoices({ q, field, from, to, limit, createdSince }) {
  ...
  if ((q || "").trim()) {
    if (field === "nic")         { where.push("i.cust_nic LIKE ?"); args.push(like); }
      ...
      /* A serial written inside a pack must be findable too. */
      where.push(`EXISTS (SELECT 1 FROM items x WHERE x.invoice_id = i.id
                    AND (x.imei LIKE ? OR x.pack_items LIKE ?))`);
      args.push(like, like);
    }
      ...
      where.push(`(i.cust_nic LIKE ? OR i.number LIKE ? OR i.cust_name LIKE ? OR i.cust_phone LIKE ?
        ...
        OR EXISTS (SELECT 1 FROM items x WHERE x.invoice_id = i.id
             AND (x.imei LIKE ? OR x.pack_items LIKE ?)))`);
  ...
  /* Cashiers see only invoices created today. created_at is ISO UTC. */
  if (createdSince) { where.push("i.created_at >= ?"); args.push(createdSince); }
...
function recordEmail(id, status, error, messageId) {
  db.prepare(`UPDATE invoices
                 SET email_status = ?, email_error = ?,
                     ...
                     email_delivered_at = CASE WHEN ? = 'sent' THEN NULL ELSE email_delivered_at END
               WHERE id = ?`)
    .run(status, error || null, status, new Date().toISOString(),
         messageId || null, status, id);
}

/* See the note in store-pg.js: matched on Brevo's message id, falling
   back to the last invoice emailed to that address. */
function recordDelivery({ messageId, email, status, reason, at }) {
  const set = `email_status = ?, email_error = ?,
               email_delivered_at = CASE WHEN ? = 'delivered' THEN ? ELSE email_delivered_at END`;
  if (messageId) {
    const rows = db.prepare(`SELECT id FROM invoices WHERE email_message_id = ?`).all(messageId);
    if (rows.length) {
      db.prepare(`UPDATE invoices SET ${set} WHERE email_message_id = ?`)
        .run(status, reason || null, status, at, messageId);
      return rows.map((r) => r.id);
    }
  }
  if (!email) return [];
  const row = db.prepare(`SELECT id FROM invoices
                           WHERE lower(cust_email) = lower(?) AND email_sent_at IS NOT NULL
                           ORDER BY email_sent_at DESC LIMIT 1`).get(email);
  if (!row) return [];
  db.prepare(`UPDATE invoices SET ${set} WHERE id = ?`)
    .run(status, reason || null, status, at, row.id);
  return [row.id];
}
...
module.exports = {
  ...
  stats:          async () => stats(),
  recordEmail:    async (id, s, e, m) => recordEmail(id, s, e, m),
  recordDelivery: async (o) => recordDelivery(o),
