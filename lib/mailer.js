...
function renderEmail(inv, attachmentName) {
  ...
  const lines = items.map((it) => {
    ...
    const w = it.warranty_type === "none"
      ? `<div style="font-size:11px;color:#5f6b7d;margin-top:4px;border-left:3px solid ${bar};padding-left:7px;line-height:1.45">${esc(wLabel("none"))}</div>`
      : Number(it.warranty_days) > 0
        ? `<div style="font-size:11px;color:#5f6b7d;margin-top:4px;border-left:3px solid ${bar};padding-left:7px;line-height:1.45">${esc(wLabel(it.warranty_type))} <b style="color:#141b25">${Number(it.warranty_days)}</b> days &middot; valid to ${niceDate(it.warranty_until)}</div>`
        : "";
    return `<tr>
...
function renderText(inv) {
  ...
  for (const it of (inv.items || [])) {
    ...
    if (it.warranty_type === "none") L.push("   No Warranty");
    else if (Number(it.warranty_days) > 0)
