...
function termListHtml(text, size) {
  ...
  return `<table cellpadding="0" cellspacing="0" width="100%">${lines.map((raw) => {
    ...
    const body = (i > 0 && i < 48)
      ...
      : esc(line);
    const cell = hot
      ? `background:#f4fbd3;color:#141b25;border-left:3px solid #c6fa02;padding:6px 8px 6px 7px`
      : `color:#5f6b7d;padding:0 0 5px`;
    return `<tr>
      <td valign="top" width="12" style="font-size:${fs2}px;color:${hot ? "#8aa800" : "#ccd3de"};${hot ? "background:#f4fbd3;padding:6px 0 6px 6px" : "padding:0 0 5px"}">&bull;</td>
      <td valign="top" style="font-size:${fs2}px;line-height:1.55;${cell}">${body}</td>
    </tr>${hot ? '<tr><td colspan="2" style="height:5px;line-height:5px;font-size:0">&nbsp;</td></tr>' : ""}`;
  }).join("")}</table>`;
}
...
function renderEmail(inv, attachmentName) {
  ...
  const showExpiry = inv.show_warranty_expiry !== false && inv.show_warranty_expiry !== 0;
  ...
  const lines = items.map((it) => {
    ...
    const w = (Number(it.warranty_days) > 0 && it.warranty_type !== "none")
      ? `<div style="font-size:11px;color:#5f6b7d;margin-top:4px;border-left:3px solid ${bar};padding-left:7px;line-height:1.45">${esc(wLabel(it.warranty_type))} <b style="color:#141b25">${Number(it.warranty_days)}</b> days${showExpiry ? ` &middot; valid to ${niceDate(it.warranty_until)}` : ""}</div>`
      : "";
    return `<tr>
...
function renderText(inv) {
  ...
  for (const it of (inv.items || [])) {
    ...
    if (Number(it.warranty_days) > 0 && it.warranty_type !== "none") {
      const exp = (inv.show_warranty_expiry !== false && inv.show_warranty_expiry !== 0)
